...
// Helper constructors and methods for cleaner actor code

// NewContractBase creates a new Contract with consistent initialization of all slices and maps
func NewContractBase(id uuid.UUID, address, name string, typ ContractType, owner string) *Contract {
	return &Contract{
		ID:      id,
		Address: address,
		Name:    name,
...
// TouchActive updates the LastActive and UpdatedAt timestamps
func (c *Contract) TouchActive() {
	c.LastActive = time.Now()
	c.UpdatedAt = c.LastActive
}

// NewContractExecution creates a new ContractExecution with proper initialization
func NewContractExecution(contractAddress, function, caller string, parameters json.RawMessage, gasLimit int64) *ContractExecution {
	return &ContractExecution{
		...
		AdaptationTriggers: make([]string, 0),
	}
}
