...
type ContractEventIntegrator struct {
	...
	contractEventSinks map[string]*ContractEventSink
...
func (cei *ContractEventIntegrator) triggerSubscriptions(event *StandardContractEvent) {
	cei.mu.RLock()
	subs := make([]*ContractEventSubscription, 0, len(cei.eventSubscriptions))
	for _, s := range cei.eventSubscriptions {
		subs = append(subs, s)
	}
	cei.mu.RUnlock()
...
func (cei *ContractEventIntegrator) matchesEventPattern(event *StandardContractEvent, pattern json.RawMessage) bool {
	...
	if types, ok := patternMap["types"].([]interface{}); ok {
		matched := false
		for _, t := range types {
			if typeStr, ok := t.(string); ok && (typeStr == "*" || typeStr == event.Type) {
				matched = true
				break
			}
		}
